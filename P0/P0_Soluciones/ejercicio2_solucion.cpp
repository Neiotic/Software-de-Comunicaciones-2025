...
void Y_recibe_y_envia_respuesta(extremo_canal & extremoY){
    ...
    ssize_t leidos = extremoY.recibe(pdu_y.data(), pdu_y.size());
    if(leidos < 2){
    ...
    if(pdu_y[0]=='C'){ //solicitud de conexion
        //respuesta:
        pdu_y[0] = 'A'; pdu_y[1]+=1;
        extremoY.envia(pdu_y.data(),2);
    }else if(pdu_y[0]=='D'){ //vienen datos!
        if(leidos > 2){ //me aseguro que hay datos en el mensaje
            std::cout << std::string_view((char *)&pdu_y[2], leidos-2); //porque sé que tienen texto (en este ejemplo)
        }
        //respuesta:
        pdu_y[0] = 'A'; pdu_y[1]+=1;
        extremoY.envia(pdu_y.data(),2);
...
int main(){
    ...
    pdu_x = {'C', seq_x++};
    extremoX.envia(pdu_x.data(),2);
    Y_recibe_y_envia_respuesta(extremoY); 

    ssize_t leidos = extremoX.recibe(pdu_x.data(), pdu_x.size());
    if(leidos < 2 || pdu_x[0]!='A' || pdu_x[1] != seq_x){
    ...
    for(auto & s : datos){
        pdu_x[1] = seq_x++;
        memcpy(pdu_x.data()+2,s.data(),s.size());
        extremoX.envia(pdu_x.data(),s.size()+2);
        Y_recibe_y_envia_respuesta(extremoY); 
    }
