...
class analiza_pcap_file{
    public:
        ...
        analiza_pcap_file(const std::string & nombre_fichero){
            ...
            fichero.read((char*)&cabecera, sizeof(cabecera));
            ...
            if(cabecera.magic_number == 0xA1B2C3D4){
            ...
            }else if(cabecera.magic_number == 0xD4C3B2A1){
        ...
        bool lee_siguiente_pdu(){
            ...
            fichero.read((char *)&cabecera_pdu, sizeof(cabecera_pdu));
            if(fichero.fail()){
                return false;
            }
            size_t tam_pdu;
            ...
            if(necesita_swap){
                tam_pdu = std::byteswap(cabecera_pdu.incl_len);
            }else{
                tam_pdu = cabecera_pdu.incl_len;
            }
            
            std::vector<uint8_t> pdu(tam_pdu); //reservo tam_pdu bytes en el heap

            //ahora se lee la pdu del fichero:
            fichero.read((char *)pdu.data(), tam_pdu);
            if(fichero.fail()){
                return false;
            }
...
int main(){
    //Ejemplo que abre un paquete de traza y muestra su contenido
