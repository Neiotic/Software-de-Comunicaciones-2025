...
public:
    ...
    */
    ssize_t recibe(void * donde, size_t maximo_de_bytes, timeval timeout){
        fd_set conjunto;
        FD_ZERO(&conjunto);
        /*POR HACER: introduce el descriptor (identificador de canal abierto) en el conjunto */
        FD_SET(canal_id, &conjunto);
        
      
        /*POR HACER: ponle los argumentos al select()*/  
        int result = select(canal_id, &conjunto, 0, 0, &timeout);
        if (result == -1) {
            // Hubo un error
            return -1;
        }else if (result == 0) {
...
bool envio_parada_y_espera(extremo_canal extremo, const std::string & mensaje){
    ...
    char ack;
    /*POR HACER: este recibe es totalmente bloqueante si no llega el ack
        (esto pasa si se pierde el mensaje que acabas de enviar o el propio ack) 
    Tienes que utilizar el método que permite recibir esperando un máximo de 2 segundos
        */
    timeval timeout = {2,0};
    ssize_t leidos = extremo.recibe(&ack,1,timeout); //no debería ser bloqueante indefinidamente
    if(leidos < 1){
        return false;
    }else{
        return true;
    ...
    if(pid > 0){ //zona exclusiva del padre
        ...
        while(!fin){
            ...
            if(mensaje!="fin"){
                ...
                //POR HACER: cuál es la condición para reintentar el envío anterior?
